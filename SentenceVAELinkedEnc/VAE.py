...
from SentenceVAELinkedEnc.ptb import PTB
from SentenceVAELinkedEnc.utils import to_var, idx2word, expierment_name
from SentenceVAELinkedEnc.model import SentenceVAELinkedEnc
...
class VAE():
	...
	def init_model_dataset(self):
		...
		for split in self.splits:
			datasets[split] = PTB(
				data_dir=f'{self.argdict["pathDataAdd"]}/SentenceVAELinkedEnc/data',
				split=split,
		...
		model = SentenceVAELinkedEnc(**params)
	...
	def augment(self):
		...
		for classes in [0,1]:
			...
			while num_to_generate>0:
				# print("Oh la la banana")
				batch_size = bs
				samples, z = self.model.inference(n=batch_size, classe=classes)
				...
				# print(z.shape)
			# print(file)
			# print(self.argdict['categories'])
			# dfs
			with open(f"{self.argdict['pathDataAdd']}/GeneratedData/VAE_LinkedEnc/{self.argdict['dataset']}/{self.argdict['dataset_size']}/{self.argdict['numFolderGenerated']}/{self.argdict['categories'][classes]}.txt", "w") as f:
				f.write(file)
