...
from CVAE.ptb import PTB
from CVAE.utils import to_var, idx2word, expierment_name
from CVAE.model import CVAE as CVAE_model
...
class CVAE():
	...
	def init_model_dataset(self):
		...
		for split in self.splits:
			datasets[split] = PTB(
				data_dir=f'{self.argdict["pathDataAdd"]}/CVAE/data',
				split=split,
	...
	def augment(self):
		...
		print(self.argdict['num_to_add'] )

		for i, catName in enumerate(self.argdict['categories']):
			num_gend=0
			file=""
			num_left=self.argdict['num_to_add']
			bs=64
			while num_left>0:
				batch_size=min([bs, num_left])
				num_left-=batch_size
				samples, z = self.model.inference(n=batch_size, cat=i)
			...
			# print(batch_size)
			# print(z.shape)
			with open(f"{self.argdict['pathDataAdd']}/GeneratedData/CVAE/{self.argdict['dataset']}/{self.argdict['dataset_size']}/{self.argdict['numFolderGenerated']}/{catName}.txt", "w") as f:
				f.write(file)
