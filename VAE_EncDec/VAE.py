...
from VAE_EncDec.ptb import PTB
from VAE_EncDec.utils import to_var, idx2word, expierment_name
from VAE_EncDec.model import SentenceVAE
import math
...
class VAE():
    ...
    def init_model_dataset(self):
        ...
        for split in splits:
            datasets[split] = PTB(
                data_dir=f'{self.argdict["pathDataAdd"]}/VAE_EncDec/data',
                split=split,
    ...
    def augment(self):
        ...
        for i in range(math.ceil(self.argdict['split'])):
            for iteration, batch in enumerate(data_loader):
                ...
                for sentence, label in zip(idx2word(samples, i2w=self.datasets['train'].get_i2w(), pad_idx=self.datasets['train'].get_w2i()['<pad>'], eos_idx=self.datasets['train'].get_w2i()['<eos>']), batch['label']):
                    files[label]+=sentence+"\n"

                # print(batch_size)
                # print(z.shape)
                for file, cat in zip(files, self.argdict["categories"]):
                    with open(f"{self.argdict['pathDataAdd']}/GeneratedData/VAE_EncDec/{self.argdict['dataset']}/{self.argdict['dataset_size']}/{self.argdict['numFolderGenerated']}/{cat}.txt", "w") as f:
                        f.write(file)
